...
test('Tag pattern without milestone', () => {
    const pattern = getTagPattern('Test_${MILESTONE}_alpha-${TAG_NUMBER}', '\\d+', undefined)
...
test('Get new initial Tag', () => {
    expect(getNewTag('Test_${MILESTONE}_rc${TAG_NUMBER}', '\\d+', 1, null, '20210101')).toBe(
        'Test_20210101_rc1'
    )
})
...
test('Get new Tag without milestone', () => {
    expect(
        getNewTag('Test_${MILESTONE}_rc${TAG_NUMBER}', '\\d+', 1, 'Test_20210101_rc2', null)
    ).toBe('Test_20210101_rc3')
    expect(
        getNewTag('Test_${MILESTONE}_${TAG_NUMBER}-alpha', '\\d+', 1, 'Test_20210101_1-alpha', null)
    ).toBe('Test_20210101_2-alpha')
})

test('Get new Tag', () => {
    expect(
        getNewTag('Test_${MILESTONE}_rc${TAG_NUMBER}', '\\d+', 1, 'Test_20210101_rc2', '20210101')
    ).toBe('Test_20210101_rc3')
    expect(
        getNewTag(
